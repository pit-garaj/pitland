	...
	<div class="stores_block_wrap">
		...
		<?foreach($arResult["STORES"] as $pid => $arProperty):
			...
			if($arParams['SHOW_EMPTY_STORE'] == 'N' && $amount <= 0)
				$empty_count++;?>
			<div class="stores_block <?=(isset($arProperty["IMAGE_ID"]) && !empty($arProperty["IMAGE_ID"]) ? 'w_image' : 'wo_image')?>" style="display: <? echo ($arParams['SHOW_EMPTY_STORE'] == 'N' && $amount <= 0 ? 'none' : ''); ?>;">
				<div class="stores_text_wrapp <?=(isset($arProperty["IMAGE_ID"]) && !empty($arProperty["IMAGE_ID"]) ? 'image_block' : '')?>">
					...
					<div class="main_info">
						<?if (isset($arProperty["TITLE"])):?>
							<span>
								<a class="title_stores" style="display:block" href="/contacts/" data-storehref="/contacts/" data-iblockhref="/contacts/"> <?=$arProperty["TITLE"].(strlen($arProperty["ADDRESS"]) && strlen($arProperty["TITLE"]) ? ', ' : '').$arProperty["ADDRESS"]?></a>
							</span>
						<?endif;?>
