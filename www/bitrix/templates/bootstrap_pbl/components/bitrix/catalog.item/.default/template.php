...
if (isset($arResult['ITEM']))
{
	...
	if ($arParams['PRODUCT_DISPLAY_MODE'] === 'N' && $haveOffers)
	{
		...
		foreach ($item['MORE_PHOTO'] as $key => $photo) {
            $renderImage = CFile::ResizeImageGet($photo["ID"], Array("width" => "300", "height" => "300"), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, false);
            $item['MORE_PHOTO'][$key]['SRC'] = $renderImage[src];
            $item['MORE_PHOTO'][$key]['WIDTH'] = $renderImage[width];
            $item['MORE_PHOTO'][$key]['HEIGHT'] = $renderImage[height];
        }
	...
	else
	{
        ...
        foreach ($actualItem['MORE_PHOTO'] as $key => $photo) {
            $renderImage = CFile::ResizeImageGet($photo["ID"], Array("width" => "300", "height" => "300"), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, false);
            $actualItem['MORE_PHOTO'][$key]['SRC'] = $renderImage[src];
            $actualItem['MORE_PHOTO'][$key]['WIDTH'] = $renderImage[width];
            $actualItem['MORE_PHOTO'][$key]['HEIGHT'] = $renderImage[height];
        }
