...
if ($arResult['MORE_PHOTO'] > 0) {
    foreach ($arResult['MORE_PHOTO'] as $key => $photo) {
        if ($photo["ID"] != 0) {
            $renderImage = CFile::ResizeImageGet($photo["ID"], Array("width" => "1400", "height" => "800"), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, false);
            $arResult['MORE_PHOTO'][$key]['SRC'] = $renderImage[src];
            $arResult['MORE_PHOTO'][$key]['WIDTH'] = $renderImage[width];
            $arResult['MORE_PHOTO'][$key]['HEIGHT'] = $renderImage[height];
        }
    }
}
...
$buyButtonClassName = in_array('BUY', $arParams['ADD_TO_BASKET_ACTION_PRIMARY']) ? 'btn-pbl' : 'btn-link';
$showAddBtn = in_array('ADD', $arParams['ADD_TO_BASKET_ACTION']);
$showButtonClassName = in_array('ADD', $arParams['ADD_TO_BASKET_ACTION_PRIMARY']) ? 'btn-pbl' : 'btn-link';
			...
			<div class="col-md-3 col-sm-12">
				<div class="row">
					<div class="col-sm-12">
									...
									case 'price':
										...
										<div class="product-item-detail-info-container price-block">
											...
											<div class="product-item-detail-price-current" id="<?=$itemIds['PRICE_ID']?>">
												<?= str_replace("руб.", "<i class='fa fa-rub' aria-hidden='true'></i>", $price['PRINT_RATIO_PRICE'] );?>  
											</div>
											<?
											if ($arParams['SHOW_OLD_PRICE'] === 'Y')
											{
                        ...
                        <?if(CSite::InDir('/catalog/tekhnika/')):?>
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            ...
                                            <h4 class="modal-title" id="myModalLabel">Запись на тест-драйв <b><?=$name?></b></h4>
                                        </div>
                                        <form class="contact">
                                            <fieldset>
                                                <div id="result"></div>
