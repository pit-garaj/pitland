	...
	foreach($productSlider as $i => $arImage){
		$productSlider[$i] = array_merge(
			$arImage, array(
				...
				"SMALL" => CFile::ResizeImageGet($arImage["ID"], array("width" => 800, "height" => 800), BX_RESIZE_IMAGE_PROPORTIONAL, true, array()),
...
if (strlen($arResult["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]) && $arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]) {
	...
	if ($arBrand) {
        ...
        if (!empty($arBrand['PREVIEW_TEXT'])) {
            $arBrand['NAME'] = strip_tags($arBrand['PREVIEW_TEXT']);
        }
	}
}
...
$videoCode = $arResult['PROPERTIES']['VIDEO_YOUTUBE']['~VALUE'][0];
if (!empty($videoCode)) {
    ...
    if(!empty($videoLink)) {
        ...
        $arResult['PROPERTIES']['VIDEO_YOUTUBE']['LINK'] = $match[1];
    }
}
...
if (!empty($arResult['MIN_PRICE']['VALUE'])) {
    ...
    $arResult['CREDIT_PRICE'] = array(
        'VALUE' => $creditPrice,
        'DISPLAY' => number_format($creditPrice, 0, ',', ' ')
    );
}
...
if ($arResult['CATALOG_WEIGHT']) {
	...
	if ($cityCode) {
		...
		$arPostData = Array(
			...
			'client_secret' => 'al9Dj88e4ROW9LjpN0gnEklnmSoZEIAm',
		);

		$curl = curl_init( 'https://api.cdek.ru/v2/oauth/token?parameters' );
		curl_setopt( $curl, CURLOPT_POST, true );
		curl_setopt( $curl, CURLOPT_POSTFIELDS, http_build_query( $arPostData ) );
		curl_setopt( $curl, CURLOPT_RETURNTRANSFER, true );
		$curlResult = curl_exec( $curl );
		curl_close( $curl );
		$arCurlResult = json_decode( $curlResult, true );
		$accessToken = $arCurlResult['access_token'];

		$jsonPostData = '{"from_location":{"code":"1100"},"to_location":{"code":"' . $cityCode . '"},"packages":[{"weight":"' . $arResult['CATALOG_WEIGHT'] . '"}]}';

		$curl = curl_init( 'https://api.cdek.ru/v2/calculator/tarifflist' );
		curl_setopt( $curl, CURLOPT_POST, true );
		curl_setopt( $curl, CURLOPT_POSTFIELDS, $jsonPostData );
		curl_setopt( $curl, CURLOPT_RETURNTRANSFER, true );
		curl_setopt( $curl, CURLOPT_HTTPHEADER, Array( 'Authorization: Bearer ' . $accessToken, 'Content-Type: application/json; charset=utf-8' ) );
		$curlResult = curl_exec( $curl );
		curl_close( $curl );
		$arCurlResult = json_decode( $curlResult, true );
		foreach ($arCurlResult['tariff_codes'] as $arTariff) {
...
if ($id = (int) $arResult['ID']) {
    ...
    $ids = array_merge([$id], $offerIds);

    $arResult['STORES_AMOUNT'] = CatalogStore::getAmount($ids);

    $storesOffersAmount = [];
    foreach ($ids as $id) {
        $storesOffersAmount[$id] = CatalogStore::getAmount([$id]);
    }
