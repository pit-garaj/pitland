...
<div class="form <?=$arResult["arForm"]["SID"]?>">
					...
					<? if ($arQuestion["STRUCTURE"][0]["FIELD_TYPE"] == "dropdown") {
						foreach ($arQuestion["STRUCTURE"] as $key => $value) {
							if ($value["MESSAGE"] == $rounded) {
								$id_select = $value["ID"];?>
								<input type="hidden" name="TIME" id="TIME" value="<?=$id_select?>">
							<?}
						}
					}?>
	...
	$(document).ready(function(){
		...
		var time = $('.CALLBACK_FOOTER #TIME').val();
		$('select[name=form_dropdown_TIME] option[value='+time+']').prop('selected', true);
