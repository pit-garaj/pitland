...
<div class="form inline <?=$arResult["arForm"]["SID"]?>">
	...
	<?if($arParams['HIDE_SUCCESS'] != 'Y' || ($arParams['HIDE_SUCCESS'] == 'Y' && !strlen($arResult["FORM_NOTE"]))):?>
		...
		<div class="form_body">
			...
			<?$rounded = date('H:i', ceil(strtotime(date("H:i"))/1800)*1800); ?>
					...
					<?foreach($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion):?>
						...
						<? if ($arQuestion["STRUCTURE"][0]["FIELD_TYPE"] == "dropdown") {
							foreach ($arQuestion["STRUCTURE"] as $key => $value) {
								if ($value["MESSAGE"] == $rounded) {
									$id_select = $value["ID"]; ?>
									<input type="hidden" name="TIME_mobile" id="TIME_mobile" value="<?=$id_select?>">
								<?}
							}
						}?>
...
<script>
	$(document).ready(function(){
		var time = $('.inline.CALLBACK_FOOTER #TIME_mobile').val();
		//console.log(time);
		$('.inline.CALLBACK_FOOTER select[name=form_dropdown_TIME] option[value='+time+']').prop('selected', true);
		$('.inline.CALLBACK_FOOTER select[name=form_dropdown_TIME]').change();
	});
</script>
