...
<?if($itemsCnt):?>
	...
	<div class="right_block1 clearfix catalog <?=strtolower($arTheme["FILTER_VIEW"]["VALUE"]);?>" id="right_block_ajax">
        ...
        <?/*<form name="_form" action="<?=$APPLICATION->GetCurDir();?>" method="get" class="smartfilter<?=$arParams['AJAX_FILTER_FLAG'];?>">
                <?echo $GLOBALS['BREND_FILTER'];?>
        </form>*/?>
        <?if($_REQUEST['ajax_get_filter']=="Y"){?>
            <script>
                $(document).ready(function(){
                    $('label.bx_filter_brand_label').on('click', function(){
                    ...
                    $('.bx_filter_brand_label').each(function(k,v){
                        if($('#' + $(v).attr('for')).prop('checked')){
                            $(v).addClass('redBorder');
                        }else{
                            $(v).removeClass('redBorder');
                        }
                    });
...
<?if($itemsCnt || true):?>
			...
			<?}?>
				...
				${$arParams["FILTER_NAME"]}['>CATALOG_PRICE_3'] = 0;
				
				$arResult['URL_TEMPLATES']['element'] = '/catalog/#SECTION_CODE_PATH#/#ELEMENT_ID#/';
				?>
